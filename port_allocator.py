...
def allocate_port(base_port, username=None, project_name=None, port_range=None):
    """
...
def main():
    parser = argparse.ArgumentParser(description='Allocate a consistent port based on username and base port')
    parser.add_argument('base_port', type=int, help='The default port number for the service')
    parser.add_argument('--username', '-u', help='Username to use (defaults to current user)')
    parser.add_argument('--project', '-p', help='Project name to make ports unique per project')
    parser.add_argument('--env-var', '-e', help='Output as export ENV_VAR=port')
    parser.add_argument('--min-port', type=int, default=1024, help='Minimum port number')
